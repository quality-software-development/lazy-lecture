...
async def send_transcriptions(message, transcriptions: list, page: int, from_callback: bool) -> None:
    ...
    for transcription in transcriptions[start_index:end_index]:
        # Create buttons for transcription, .txt, and .docx
        ...
        txt_button = InlineKeyboardButton(text=".txt", callback_data=f"download_txt_{transcription['task_id']}")
        docx_button = InlineKeyboardButton(text=".docx", callback_data=f"download_docx_{transcription['task_id']}")

        # Add a new row for each transcription
        # keyboard.append([transcription_button, txt_button, docx_button])
