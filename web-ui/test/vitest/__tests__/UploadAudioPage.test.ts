...
import { router, getMockUser } from 'test/vitest/setup-file';
...
describe('Страница отправки аудио на обработку', () => {
    it('Обработка аудио должна быть недоступна, если у пользователя нет соответствующих прав', async () => {
        ...
        expect(wrapper.vm.$refs.uploader).toBeNull();
        expect(
            wrapper
                .get('[data-test="ui-testing-upload-audio-page-forbidden-p"]')
                .text()
        ).toBe(
            'Нет доступа к обработке аудио.Запросите доступ у администраторов'
        );
    });
    it('Обработка аудио должна быть недоступна, если уже есть аудио в обработке', async () => {
        ...
        expect(
            wrapper
                .get('[data-test="ui-testing-upload-audio-page-processing-p"]')
                .text()
    ...
    it('Аудио тяжелее 200Мб - должно быть выведено сообщение об ошибке', async () => {
        ...
        const uploader = wrapper.vm.$refs.uploader as QUploader;
    ...
    it('При отправке аудио должен начаться опрос состояния обработки', async () => {
        ...
        await (wrapper.vm as any).handleAudioUpload();
