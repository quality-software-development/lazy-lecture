...
#     assert response.status_code == 200 and "access_token" in response.json(), response.json()
#     headers = {"Authorization": f"Bearer {response.json().get('access_token')}"}
#     payload = {"transcription_id": 1, "current_state": "completed", "new_chunk": {"text": "Test chunk", "chunk_no": 1}}
#     try:
#         response = await async_client.post(
#             "/worker/transcription_status?secret_worker_token=71y209716yc20n971yoj", json=payload, headers=headers
#         )
#     except:
#         assert True
#     # assert response.status_code == 422
...
#     headers = {"Authorization": f"Bearer {response.json().get('access_token') + '1'}"}
#     monkeypatch.setattr(
#         "source.app.transcriptions.services.update_transcription_state", fake_update_transcription_state
#     )
#     payload = {"transcription_id": 1, "current_state": "completed", "new_chunk": {"text": "Test chunk", "chunk_no": 1}}
#     try:
#         response = await async_client.post(
