...
class LazyLectureWorker:
    ...
    async def process_message(self, message: IncomingMessage):
        ...
        try:
            ...
            if transcription_info.current_state in TERMINAL_STATES:
                await message.nack(requeue=False)
                self.logger.info("Task nacked due to being in terminal state already")
                return
            elif transcription_info.current_state == TranscriptionState.PROCESSING_ERROR:
                if transcription_info.error_count >= 3:
                ...
                await message.nack(requeue=False)
                return
            elif transcription_info.current_state == TranscriptionState.QUEUED:
                self.api_client.update_transcription_state(task_data.transcription_id, TranscriptionState.IN_PROGRESS)
            else:
                self.logger.info("Incorrect state of task encountered. Moving object to processing fail state.")
                self.api_client.update_transcription_state(
                    task_data.transcription_id, TranscriptionState.PROCESSING_FAIL
                )
                await message.nack(requeue=False)
            ...
            # Process audio
            user_audio = self.object_storage.get_user_audio(task_data.user_id)
            ...
            audio_len_secs: float = transcription_info.audio_len_secs
            chunk_size_secs: int = transcription_info.chunk_size_secs
            self.logger.info(f"Audio length is {audio_len_secs:.2f} seconds")
            self.logger.info("Inference Begin!")

            clip_timestamps = [
                [v, min(audio_len_secs, v + chunk_size_secs)]
                for v in range(0, int(audio_len_secs) + 1, chunk_size_secs)
            ]
            clip_timestamps[-1] = [
                clip_timestamps[-1][0],
            ]
            ...
            for chunk_no, clip_timestamp in enumerate(clip_timestamps):
                ...
                new_chunk = TranscriptionChunk(text=text, chunk_no=chunk_no)
                ...
                if transcription_info.current_state in [
                    ...
                    self.api_client.update_transcription_state(
                        task_data.transcription_id, TranscriptionState.COMPLETED_PARTIALLY, new_chunk=new_chunk
                    )
                    self.logger.info("Task was cancelled, nacking")
                    await message.nack(requeue=False)
                    return
                else:
                    new_state = TranscriptionState.COMPLETED if chunk_no == len(clip_timestamps) - 1 else None
                    self.api_client.update_transcription_state(
                        task_data.transcription_id, new_state=new_state, new_chunk=new_chunk
                    )
    ...
    def _infer_chunk(self, user_audio: Path, clip_timestamp: Union[str, List[float]], audio_len_secs: int) -> str:
        ...
        text, _ = self.asr_predictor.transcribe_audio_file(user_audio, clip_timestamp)
